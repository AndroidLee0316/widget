package com.pasc.lib.widget.pickerview.view;

import android.graphics.Typeface;
import android.view.View;

import com.pasc.lib.widget.R;
import com.pasc.lib.widget.pickerview.adapter.ArrayWheelAdapter;
import com.pasc.lib.widget.pickerview.lib.WheelView;
import com.pasc.lib.widget.pickerview.listener.OnItemSelectedListener;
    ...
    public void setPicker(List<T> options1Items,
        ...
        wheelListener_option1 = new OnItemSelectedListener() {
            ...
            public void onItemSelected(int index) {
                int opt2Select = 0;
                if(index != -1 && mOptions2Items.get(index) != null){

                    opt2Select = wv_option2.getCurrentItem();//上一个opt2的选中位置
                    //新opt2的位置，判断如果旧位置没有超过数据范围，则沿用旧位置，否则选中最后一项
                    int selectPosition = 0;
                        selectPosition = mOptions2Items.get(index).size() - 1;
                        opt2Select = opt2Select >= selectPosition ? selectPosition : opt2Select;
                    ...
                    newList.add("");
                    wv_option2.setAdapter(new ArrayWheelAdapter(newList));
                    wv_option2.setCurrentItem(-1);
                    if (mOptions3Items != null) {
                        wheelListener_option2.onItemSelected(-1);
                    }
                }
...
//                }
        ...
        wheelListener_option2 = new OnItemSelectedListener() {
            ...
            public void onItemSelected(int index) {
                if(index == -1){
                    List<String> newList =new ArrayList<>();
                    newList.add("");
                    wv_option3.setAdapter(new ArrayWheelAdapter(newList));
                    wv_option3.setCurrentItem(-1);
                }else {
                    if (mOptions3Items != null) {

                        int k = wv_option1.getCurrentItem();
                        if(mOptions3Items.size()>k){
                            ...
                            if(mOptions3Items.get(opt1Select).size()> index && index != -1){
    ...
    public int[] getCurrentItems() {
        ...
        if (mOptions2Items != null && mOptions2Items.size() > 0) {//非空判断
            if( mOptions2Items.get(currentItems[0]) != null){
        ...
        if (mOptions3Items != null && mOptions3Items.size() > 0) {//非空判断
            if(mOptions3Items.size()> currentItems[0]){
                if(mOptions3Items.get(currentItems[0]) != null){
                    if(mOptions3Items.get(currentItems[0]).size() >currentItems[1]){
    ...
    private void itemSelected(int opt1Select, int opt2Select, int opt3Select) {
        boolean hasOpt2Select = false;
        if (mOptions2Items != null) {
            if(mOptions2Items.get(opt1Select) != null && mOptions2Items.get(opt1Select).size() >0){
                hasOpt2Select = true;
        ...
        if (mOptions3Items != null && hasOpt2Select) {
            if(mOptions3Items.size() > opt1Select){
                    if(mOptions3Items.get(opt1Select).size() >opt2Select){
                        if(mOptions3Items.get(opt1Select).get(opt2Select).size()> 0){
                            wv_option3.setAdapter(new ArrayWheelAdapter(mOptions3Items
                                    .get(opt1Select).get(
                                            opt2Select)));
