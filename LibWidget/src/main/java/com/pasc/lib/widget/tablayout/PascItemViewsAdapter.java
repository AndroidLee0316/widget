 ...
 * 复用已存在的 {@link View}。
 * 用于 {@link PascTabLayout} 中 {@link PascTab} 与数据的适配。
 */

public abstract class PascItemViewsAdapter<T, V extends View> {
    ...
    public PascItemViewsAdapter(ViewGroup parentView) {
    ...
    public void detach(int count) {
        ...
        while (childCount > 0 && count > 0) {
            ...
            Object notCacheTag = view.getTag(R.id.pasc_view_can_not_cache_tag);
    ...
    public PascItemViewsAdapter<T, V> addItem(T item) {
        mItemData.add(item);
        return this;
    }
