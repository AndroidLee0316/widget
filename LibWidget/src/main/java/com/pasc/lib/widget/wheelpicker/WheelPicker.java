...
//    private static final String TAG = WheelPicker.class.getSimpleName();
    ...
    public WheelPicker(Context context, AttributeSet attrs) {
        ...
        mItemTextSize = a.getDimension(R.styleable.WheelPicker_wheel_item_text_size,
                DensityUtils.dp2px(16));
        ...
        mItemTextColor = a.getColor(R.styleable.WheelPicker_wheel_item_text_color, getResources().getColor(R.color.wheel_picker_item_text_color));
        mItemSpace = a.getDimension(R.styleable.WheelPicker_wheel_item_space,
        ...
        mIndicatorColor = a.getColor(R.styleable.WheelPicker_wheel_indicator_color, getResources().getColor(R.color.wheel_picker_indicator_color));
        mIndicatorSize = a.getDimension(R.styleable.WheelPicker_wheel_indicator_size,
                DensityUtils.dp2px(2));
        mIndicatorMarginLeft = (int) a.getDimension(R.styleable.WheelPicker_wheel_indicator_marginLeft, 0);
        mIndicatorMarginRight = (int) a.getDimension(R.styleable.WheelPicker_wheel_indicator_marginRight, 0);
        hasCurtain = a.getBoolean(R.styleable.WheelPicker_wheel_curtain, false);
        mCurtainColor = a.getColor(R.styleable.WheelPicker_wheel_curtain_color, getResources().getColor(R.color.wheel_picker_curtain_color));
...
//        mRectIndicatorFoot = new Rect();
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int resultHeight = (int) (mTextMaxHeight * mVisibleItemCount + mItemSpace * (mVisibleItemCount - 1));
...
//        int halfIndicatorSize = (int) (mIndicatorSize / 2);
//        int indicatorHeadCenterY = mWheelCenterY + mHalfItemHeight + DensityUtils.dp2px(5);
//        int indicatorFootCenterY = mWheelCenterY - mHalfItemHeight - DensityUtils.dp2px(5);
//        mRectIndicatorHead.set(mRectDrawn.left + mIndicatorMarginLeft, indicatorHeadCenterY - halfIndicatorSize,
//                mRectDrawn.right - mIndicatorMarginRight, indicatorHeadCenterY + halfIndicatorSize);
//        mRectIndicatorFoot.set(mRectDrawn.left + mIndicatorMarginLeft, indicatorFootCenterY - halfIndicatorSize,
//                mRectDrawn.right - mIndicatorMarginRight, indicatorFootCenterY + halfIndicatorSize);
//    }
    ...
    private void drawLine(Canvas canvas) {
        ...
        linePaint.setColor(mIndicatorColor);
        linePaint.setAntiAlias(true);
        linePaint.setStrokeWidth(mIndicatorSize);

        //无外边距时，调整长度以隐藏圆角
        if (mIndicatorMarginLeft == 0)
            mIndicatorMarginLeft = -DensityUtils.dp2px(2);
        if (mIndicatorMarginRight == 0)
            mIndicatorMarginRight = -DensityUtils.dp2px(2);

        //上指示器
        RectF upRectF = new RectF(mIndicatorMarginLeft, mHalfWheelHeight - (mHalfItemHeight + DensityUtils.dp2px(0)) - mIndicatorSize / 2,
                mRectDrawn.right - mIndicatorMarginRight, mHalfWheelHeight - (mHalfItemHeight + DensityUtils.dp2px(0)) + mIndicatorSize / 2);
        canvas.drawRoundRect(upRectF, 2.5f, 2.5f, linePaint);   //圆角标记线

        //下指示器
        RectF downRectF = new RectF(mIndicatorMarginLeft, mHalfWheelHeight + (mHalfItemHeight + DensityUtils.dp2px(0)) - mIndicatorSize / 2,
                mRectDrawn.right - mIndicatorMarginRight, mHalfWheelHeight + (mHalfItemHeight + DensityUtils.dp2px(0)) + mIndicatorSize / 2);
        canvas.drawRoundRect(downRectF, 2.5f, 2.5f, linePaint);
    }

    private void computeCurrentItemRect() {
        if (!hasCurtain && mSelectedItemTextColor == -1) return;
        mRectCurrentItem.set(mRectDrawn.left, mWheelCenterY - mHalfItemHeight - DensityUtils.dp2px(5), mRectDrawn.right,
                mWheelCenterY + mHalfItemHeight + DensityUtils.dp2px(5));
    }
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (Math.abs(velocity) > mMinimumVelocity) {
                    ...
                    int distance = isClick ? (int) event.getY() - getHeight() / 2 : 0;
                    mScroller.startScroll(0, mScrollOffsetY - distance, 0,
                            computeDistanceToEndPoint((mScrollOffsetY - distance) % mItemHeight));
                }
                // 校正坐标
                // Correct coordinates
                if (!isCyclic)
