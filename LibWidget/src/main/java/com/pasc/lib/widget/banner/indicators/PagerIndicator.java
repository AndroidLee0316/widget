...
public class PagerIndicator extends LinearLayout implements ViewPagerEx.OnPageChangeListener {
    ...
    public enum Shape {
        Oval, Rectangle, SelectOval, SelectRectangle
    }
    ...
    public void setDefaultIndicatorShape(Shape shape) {
        ...
        if (mUserSetUnSelectedIndicatorResId == 0) {
            if (shape == Shape.SelectRectangle) {
                mSelectedGradientDrawable.setShape(GradientDrawable.OVAL);
                mUnSelectedGradientDrawable.setShape(GradientDrawable.RECTANGLE);
            }
        }
    ...
    /**
     * since we used a ListAdapter wrapper, so we can't getCount directly from wrapper.
     *
     * @return
     */
    ...
    private void setItemAsSelected(int position) {
        ...
        if (childCount > 0) {
            int currentPosition = position % childCount;
            ImageView currentSelected = (ImageView) getChildAt(currentPosition);
            ...
            mPreviousSelectedPosition = currentPosition;
        }
    }
